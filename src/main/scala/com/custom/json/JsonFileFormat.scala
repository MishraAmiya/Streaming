...
import com.com.custom.json.catalyst.json.{JSONOptions, JacksonGenerator, JacksonParser}
...
class JsonFileFormat extends TextBasedFileFormat with DataSourceRegister {
  ...
  override def buildReader(
    ...
    dataSchema.fields.map(_.name).zipWithIndex.toMap.get(parsedOptions.columnNameOfCorruptRecord).foreach { corruptFieldIndex =>
      val f = dataSchema(corruptFieldIndex)
      if (f.dataType != StringType || !f.nullable) {
        throw new CustomAnalysisException(
...
class SerializableConfiguration(@transient var value: Configuration) extends Serializable {
  private def writeObject(out: ObjectOutputStream): Unit = JsonUtils.tryOrIOException {
    out.defaultWriteObject()
    value.write(out)
  }

  private def readObject(in: ObjectInputStream): Unit = JsonUtils.tryOrIOException {
    value = new Configuration(false)
    value.readFields(in)
  }
}
class CustomAnalysisException(message: String) extends AnalysisException(message)