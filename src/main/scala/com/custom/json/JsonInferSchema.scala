...
object JsonInferSchema {
  ...
  def infer[T](
    ...
    val rootType = json.mapPartitions { iter =>
      ...
      iter.flatMap { row =>
        try {
          JsonUtils.tryWithResource(createParser(factory, row)) { parser =>
            parser.nextToken()
  ...
  def compatibleType(t1: DataType, t2: DataType): DataType = {
    TypeCoercion.findTightestCommonType(t1, t2).getOrElse {
      ...
      (t1, t2) match {
        ...
        case (t1: NumericType, t2: DecimalType) =>
          compatibleType(CustomDecimalType.forType(t1), t2)
        case (t1: DecimalType, t2: NumericType) =>
          compatibleType(t1, CustomDecimalType.forType(t2))
...
object CustomDecimalType extends DecimalType {
