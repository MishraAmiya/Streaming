...
import com.custom.json.JsonUtils
...
class JacksonParser(
    ...
    case (str:StringType,md:Metadata) =>
      (parser: JsonParser) => parseJsonToken[UTF8String](parser, dataType) {
        ...
        case _ =>
          ...
          JsonUtils.tryWithResource(factory.createGenerator(writer, JsonEncoding.UTF8)) {
    ...
    case (date:DateType,md:Metadata) =>      
      ...
      (parser: JsonParser) => parseJsonToken[java.lang.Integer](parser, dataType) {
        case VALUE_STRING =>
          ...
          Int.box {
            Try(DateTimeUtils.millisToDays(dateFormat.parse(stringValue).getTime))
              .orElse {
                // If it fails to parse, then tries the way used in 2.0 and 1.x for backwards
                // compatibility.
  ...
  private def convertObject(
    ...
    while (nextUntil(parser, JsonToken.END_OBJECT)) {
      schema.fields.map(_.name).zipWithIndex.toMap.get(parser.getCurrentName) match {
        case Some(index) =>
  ...
  def parse[T](
    ...
    try {
      JsonUtils.tryWithResource(createParser(factory, record)) { parser =>
